...
class Item {
    ...
    public DOM:objectype;
    private options:objectype;
    private mouseenterFn:Function=()=>{};
    private mouseleaveFn:Function=()=>{};
    private mousetime:number=0;
    private mousemoveFn:Function=()=>{};
    private allowTilt = true;
    constructor(el:HTMLElement, options:objectype = {}) {
        this.DOM = {el: el};
    ...
    initEvents() { 
        ...
        this.mouseleaveFn = (ev:MouseEvent) => requestAnimationFrame(() => {
            if ( !enter || !this.allowTilt ) return;
            enter = false;
            clearTimeout(this.mousetime);
...
class Overlay {
    ...
    revealItem() {
        (this.contentItem as HTMLElement).style.opacity = "1";
...
export class Grid {
    ...
    constructor(el:HTMLElement) {
        ...
        Array.from(this.DOM.el.querySelectorAll('a.grid__item')).forEach((item) => {
            const itemObj = new Item(item as HTMLElement);
            this.items.push(itemObj);
            if ( !(item as HTMLElement).classList.contains('grid__item--noclick') ) {
                itemObj.DOM.el.addEventListener('click', (ev:Event) => {
                    ev.preventDefault();
                    this.openItem(document.querySelector((item as any)?.getAttribute('href')??""));
                });
            }
        });
    ...
    openItem(contentItem:HTMLElement) {
